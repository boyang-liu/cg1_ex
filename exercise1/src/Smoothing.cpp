...
void SmoothUniformLaplacian(HEMesh& m, float lambda, unsigned int iterations)
{
	...
	for (auto v : m.vertices())
	{
		...
		OpenMesh::Vec3f v_delta(0.0, 0.0, 0.0);
		for (int i=0;i<v_vh.size();i++) 
		{
				
			v_delta = v_delta +( m.point(v_vh[i])-m.point(v));

		}
		v_delta = v_delta / v_vh.size();
		v_new.push_back(lambda * v_delta)  ;
	}
	int k = 0;
	for (auto v1 : m.vertices()) 
	{
		m.point(v1) = m.point(v1) + v_new[k];
			k++;
	}
	}
}
...
void SmoothcotangentLaplacian(HEMesh& m, float lambda)//, float lambda, unsigned int iterations
