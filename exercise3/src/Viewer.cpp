...
GLuint CreateTexture(const unsigned char* fileData, size_t fileLength, bool repeat = true)
{
	...
	//glUniform1i(glGetUniformLocation(textureName, "diffuseTexture"), 0);
...
void Viewer::CreateGeometry()
{
	...
	{
		int base = x * PATCH_SIZE;
		for (int z = 0; z < PATCH_SIZE-1; z++) 
		{
			
			indices.push_back((uint32_t)(base + z));
			indices.push_back((uint32_t)(base + PATCH_SIZE + z));
			indices.push_back((uint32_t)(base + z+1));

			indices.push_back((uint32_t)(base + z+1));
			indices.push_back((uint32_t)(base + PATCH_SIZE + z ));
			indices.push_back((uint32_t)(base + PATCH_SIZE + z + 1));
	...
	offsetBuffer.bind();
	//terrainShader.attrib("offset");
	//my_offsets.push_back({ 0.0, 0.0 });
	offsetBuffer.uploadData(my_offsets).bindToAttribute("offset");/**/
...
void Viewer::drawContents()
{
	...
	terrainShader.setUniform("diffuseTexture", 0);

	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, rockTexture);
	terrainShader.setUniform("diffuseTexture_2", 1);

	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, roadColorTexture);
	terrainShader.setUniform("diffuseTexture_3", 2);


	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, alphaMap);
	terrainShader.setUniform("diffuseTexture_4", 3);

	glActiveTexture(GL_TEXTURE4);
	glBindTexture(GL_TEXTURE_2D, roadSpecularMap);
	terrainShader.setUniform("diffuseTexture_5", 4);

	
	glActiveTexture(GL_TEXTURE5);
	glBindTexture(GL_TEXTURE_2D, roadNormalMap);
	terrainShader.setUniform("diffuseTexture_6", 5);
	...
	glDrawElementsInstanced(GL_TRIANGLES, myindices.size(), GL_UNSIGNED_INT, (void*)0, 1);
	//Render text
