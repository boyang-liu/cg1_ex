...
public:
	...
	ResultEntry find_node(AABBNode* mynode, const Eigen::Vector3f& q) const 
	{
		if (mynode->IsLeaf()) {
			AABBLeafNode *thisLeafNode = (AABBLeafNode *)mynode;


			ResultEntry smaller_one;
			for (auto pit = thisLeafNode->begin(); pit != thisLeafNode->end(); ++pit) {
				float dist = pit->SqrDistance(q);
				if (dist < smaller_one.sqrDistance)
				{
					smaller_one.sqrDistance = dist;
					smaller_one.prim = &(*pit);
				}
			}
			return smaller_one;
			
		}
		ResultEntry closestone;
		AABBSplitNode* thisnode = (AABBSplitNode *)mynode;
		AABBNode* ll = thisnode->Left();
		AABBNode* rr = thisnode->Right();
		Box leftbox = ll->GetBounds();
		Box rightbox = rr->GetBounds();
		if (leftbox.SqrDistance(q)> rightbox.SqrDistance(q)) {
			closestone = find_node(thisnode->Right(), q);
		}else{
			closestone = find_node(thisnode->Left(), q);
		}
		return closestone;
	...
	ResultEntry ClosestPrimitiveAABBTreeSearch(const Eigen::Vector3f& q) const
	{
		ResultEntry best = find_node(root, q);
		return best;
	}
	...
	ResultEntry ClosestPrimitive(const Eigen::Vector3f& q) const
	{
		...
		return  ClosestPrimitiveAABBTreeSearch(q);
	}
